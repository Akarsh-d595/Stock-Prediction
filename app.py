...
def search_ticker_by_name(company_name):
    ...
    try:
        ...
        st.error(f"Error during name-to-ticker lookup: {e}")
        return None
...
company_name_input = st.text_input("Or enter Company Name (less reliable)", "")

if st.button("Predict"):
    selected_ticker = ticker_input

    if not ticker_input and company_name_input:
        # Attempt to get ticker from company name if no ticker is provided
        st.info(f"Attempting to find ticker for '{company_name_input}'...")
        # As discussed, direct yfinance search is not robust for general names.
        # This will trigger the warning in search_ticker_by_name
        selected_ticker = search_ticker_by_name(company_name_input)
        if selected_ticker is None:
            st.warning("Please provide a direct ticker symbol for better results.")
            st.stop()

    if not selected_ticker:
        st.warning("Please enter either a stock ticker symbol or a company name.")
    else:
        st.subheader(f"Analyzing {selected_ticker}...")
            ...
            if df_features.empty:
                ...
                if df_model_ready.empty:
                    ...
                    with col_pred_metric:
                        st.metric(label=f"Predicted Close Price for {selected_ticker}", value=f"${predicted_next_day_close:.2f}", delta_color="off")
                    with col_pred_info:
                    ...
                    # Volume Bar Chart
                    st.plotly_chart(plot_volume(df, selected_ticker), use_container_width=True)

                    # Comparison Plot (Actual vs. Historical Predicted)
                    st.plotly_chart(plot_comparison(df_model_ready, df_model_ready['Historical_Predicted_Close'], selected_ticker), use_container_width=True)
        else:
